 ...
 * Geometry tile layer
 */
export class GeoTileLayer extends Entity {
  
  /**
   * Creates a new GeoTileLayer
  ...
  constructor(public name?: string, public description?: string, public templateUrl?: string, public tileLayerOptions?:{}) {
    super(ENTITY_MODELS.GEOMETRY.GEO_TILE_LAYER, GeoTileLayer.createId(name), true);
  }
   ...
   */
  public static mapToEntity(source: GeoTileLayer | Entity): GeoTileLayer {
    let r: GeoTileLayer = Object.assign(new GeoTileLayer(), source);
    return r;
  }

  public static mapToEntityArray(source: GeoTileLayer[]): GeoTileLayer[] {
    if (source.length < 1)
      return [];
    let array = [];
    source.forEach(r => {
